...
Monorepo **Jobless**（`WeKruit/Jobless`）：**Mac + Photon (iMessage)** 作为唯一 Messages 出口；**Firebase**（`wekruit-5f89b`）存 `pa_users` / `pa_sessions` / `pa_messages` / `pa_agents` / `pa_remote_config` / **`pa_outbound`**；**operator dashboard**（Vite + React）在 **`/playground`** 管理用户、agent、outbound（**Refresh**，无 SSE）。LLM/ATM/Infisical 与 VALET 对齐（`config/ATM.md`、`config/ENV.md`）。

## 已实现要点

1. iMessage 1:1 会话默认 **E.164** 为 `externalChatId`；`PA_IMESSAGE_SESSION_KEY=chatid` 为旧行为。  
2. **`pa_outbound`** + macOS worker Firestore listener → Photon send + `appendMessage`。  
3. **`/playground`** + Firestore rules + `pa_outbound` composite index。  
4. Worker：`dotenv` 读 `apps/macos-imessage-worker/.env`。  
...
### F. Mac worker（真机 iMessage / outbox）

1. **系统设置 → 隐私 → 完全磁盘访问**：给跑 worker 的终端/Node 授权。  
2. `apps/macos-imessage-worker/.env`（或 Infisical）：服务账号、Firestore、`ATM_*` / `OPENAI_*` 等（见 `LOCAL.md`）。  
3. `cd apps/macos-imessage-worker && npm run start`。  
4. Dashboard **`/playground`**：enqueue outbound → worker 应把状态从 `pending` 推到 `sent`（页面 **Refresh**）。
...
| Outbound 一直 `pending` | Worker 是否在跑、索引是否已 deploy |
...
# 本地
cd apps/macos-imessage-worker && npm run start
...
- Worker：[`apps/macos-imessage-worker/src/index.ts`](../apps/macos-imessage-worker/src/index.ts)  
- Outbox：[`apps/macos-imessage-worker/src/outbox.ts`](../apps/macos-imessage-worker/src/outbox.ts)  
...
架构：Firestore pa_* + pa_outbound；Mac worker（Photon iMessage）+ Dashboard（Vite/React，/playground）。Hosting 独立站点 wekruit-pa（https://wekruit-pa.web.app），firebase target pa-dashboard；deploy 勿用默认 wekruit-5f89b Hosting。
...
6) Mac：Full Disk Access + worker .env + npm run start。

必读：config/HANDOFF.md（顺序清单）、config/ENV.md、config/DOMAIN.md、config/LOCAL.md。

验证：typecheck；dig/curl 域名；浏览器无 VITE 报错；Playground outbound pending→sent（worker 运行中）。
```
