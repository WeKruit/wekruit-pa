...
The PA **worker** can call Mem0 when an agent’s **`memoryMode`** is `mem0` or `both` (see [ENV.md](ENV.md) and `packages/pa-memory`).

## What you need to self-host Mem0

1. A running **Mem0 API** (official Docker/helm or your deployment) with a **base URL** and **API key**.  
2. On the **Mac worker** environment (not the browser):  
   - `MEM0_API_KEY`  
...
4. If Mem0 is down, the worker logs **degraded** and can continue in **firestore-only**-style behavior depending on code paths — for a predictable POC, use **`firestore_only`** until Mem0 is stable.
...
4. After deploy, `https://<your-mem0-app>.fly.dev` (or a custom domain) is your `MEM0_BASE_URL` on the **Mac worker**.  
5. Ensure the worker can reach the Fly app (public HTTPS is simplest; private WireGuard from Mac to Fly is more setup).
