...
export function buildExtractorPrompt(req: ConversationExtractRequest): string {
  ...
  return [
    ...
    "    Claire can recall the original phrasing later. Each memoryEntities row MUST",
    "    include ALL FOUR fields: entityKind (the tagPatch field name like industrySector",
    "    or targetRoleFunction), value (the canonical token), confidence (number 0..1),",
    "    and evidence (a short verbatim quote from the user). Missing any field is a",
