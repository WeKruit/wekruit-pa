  ...
  switch (mode) {
    ...
    default:
      return [
        ...
        "passing for now (you'll send more). When they pick a role to prescreen, relay THAT role's",
        "'[start prescreen — copy & reply this exact line] WeKruit_..._Job' token VERBATIM (the right role — never a",
        "different one; clarify first if unsure). Do NOT roleplay starting it or ask for their email. Only the",
