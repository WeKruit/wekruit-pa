...
const DELIVERY = [
  ...
  "  not a job board.",
  "- OFFERING THE PRESCREEN (do NOT ask them to rank/prioritize roles): after you share the collab roles,",
  "  do NOT ask 'which one should I prioritize'. Instead offer the prescreen as a low-pressure choice: ask",
  "  if they want to knock out a quick prescreen now (so we can pitch them faster and the role moves faster",
  "  to them) — and reassure them that if not, that's totally fine, they can pass and you'll keep sending",
  "  more opportunities. Let THEM tell you which role they're interested in.",
  ...
  switch (mode) {
    ...
    default:
      return [
        ...
        "can just NAME the role in plain words ('let's do the Helium one', 'start the invoko PM screen') — when",
        "they do, CALL begin_collab_prescreen with roleQuery = what they said, and it starts that one role's",
        "interview for you. If begin_collab_prescreen returns reason 'no_match' or 'ambiguous', DO NOT start",
        "anything and NEVER start the wrong role — re-offer the roles (no_match) or ask which one they mean",
        "(ambiguous), then try again. Do NOT roleplay starting it or ask for their email (the session handles",
        "that). Only the marked collab/partner roles get this pitch; share open-market roles normally.",
        "PROGRESS / STATUS QUESTIONS — when the candidate asks how a screen is going or whether they passed",
        "('how did my screen go?', 'did I pass the Invoko one?', 'status of my interviews?'), CALL",
        "check_prescreen_progress (pass query = the company/title if they named one, else leave it null) and",
        "relay what it returns (passed / not_passed / in_progress) warmly — do NOT guess or invent an outcome.",
