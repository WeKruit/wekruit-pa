 ...
 *
 * Adam top-level-design fix (2026-05-03): we already receive `typing_indicator`
...
const LOVE_TAPBACK_INELIGIBLE_RE =
  /\b(no|cannot|can't|cant|do not|does not|did not|don't|doesn't|didn't|have not|has not|haven't|hasn't|never|not able|not done|not built|not shipped|unable|won't|would not|without|not a fit|not aligned|too low|no experience|no .* experience|only support|support-only|need remote|remote only|only remote|relocat(?:e|ion)?|onsite|weekly travel|sponsorship|visa|h-?1b|qa canary|canary|ignore this test message|rate limit|recommend(?:ed)? (?:two|three|\d+)? ?(?:frontend|fullstack|software|job|role|roles|jobs)|job recommendations?|role recommendations?|what data do you store|can i see what you remember|what you remember|my memory|export my data|delete my data|privacy|ignore (?:all )?(?:previous|prior) instructions|reveal (?:your )?(?:system|developer)? ?prompt|system prompt|developer prompt|internal instructions|jailbreak|DAN|developer message|another(?:\s+[a-z0-9&.-]+){0,3}\s+candidate(?:'s|’s)?\s+(?:resume|cv|profile|notes?|data|information|contact|email|phone)|other(?:\s+[a-z0-9&.-]+){0,3}\s+candidate|candidate.?s resume)\b/i
