/**
 * Stream H10 D3 — rematch driver. Forked from H8 (cosine + anti-bias), now
...
const REMATCH_SUFFIX = "batch-h10-rematch";
...
console.log("[h10-rematch] starting", JSON.stringify(args));
...
console.log("[h10-rematch] resolved users:", users);
...
const ymd = todayYmd();
const log = (...a) => console.log("[h10-rematch]", ...a);
...
async function rematchOne(userId) {
  ...
  if (sendRes.ok && sendRes.messageHandle) {
    await db.collection("pa-outbound").doc(sendRes.messageHandle).set({
      rematchReason: "stream-h10-cross-encoder-2026-05-01",
...
for (const u of users) {
  try { results.push(await rematchOne(u)); }
    ...
    console.error("[h10-rematch] FAIL for", u, err);
    results.push({ userId: u, error: err instanceof Error ? err.message : String(err) });
  }
}

console.log("\n" + "=".repeat(72));
console.log("Stream H10 D3 — rematch summary");
