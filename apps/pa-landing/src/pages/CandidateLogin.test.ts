...
test("CandidateLogin frames onboarding login as a first-time Claire start", () => {
  ...
  assert.match(source, /onboardingNext[\s\S]*isYcStartupLogin[\s\S]*\? "Startup School people matching"[\s\S]*: "Start with Claire"/)
  assert.match(source, /onboardingNext[\s\S]*isYcStartupLogin[\s\S]*\? <>Start <em className="wk-accent">yapping\.<\/em><\/>[\s\S]*: <>Start with <em className="wk-accent">Claire\.<\/em><\/>/)
...
test("CandidateLogin keeps onboarding auth controls before the explanatory preview", () => {
  ...
  assert.match(source, /const isYcStartupLogin = loginSignupSource === "yc_startup_school"/)
  assert.match(source, /fromQuery === "yc_startup_school"/)
  ...
  const contextStripIndex = source.indexOf("<LoginContextStrip kind={loginContextKind} />")
  const onboardingPreviewIndex = source.indexOf("{showOnboardingPreview ? <LoginOnboardingPreview isYcStartup={isYcStartupLogin} /> : null}")
