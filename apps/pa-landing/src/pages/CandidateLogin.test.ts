...
test("CandidateLogin keeps onboarding auth controls before the explanatory preview", () => {
  ...
  assert.match(source, /onboarding: \[[\s\S]*\{ title: "Profile home", body: "Resume, LinkedIn, targets, corrections\." \}/)
  assert.match(source, /\{ title: "Active screens", body: "Claire turns fit into interview evidence\." \}/)
  assert.match(source, /\{ title: "Consent gate", body: "Only passed evidence can be shared\." \}/)
  ...
  assert.match(source, /@media \(max-width: 480px\)[\s\S]*\.wk-login-context__items \{ grid-template-columns: 1fr; gap: 8px; \}/)
  assert.match(source, /@media \(max-width: 480px\)[\s\S]*\.wk-login-context__item em \{ font-size: 12px; line-height: 1\.3; \}/)
