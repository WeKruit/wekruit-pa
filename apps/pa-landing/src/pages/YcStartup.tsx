...
const SOURCE = "yc_startup_school"
const ONBOARDING_HREF = `/onboarding?source=${SOURCE}`
const LOGIN_HREF = `/login?next=${encodeURIComponent(ONBOARDING_HREF)}`
const TICKER_COPY = "STARTUP SCHOOL ATTENDEES x WEKRUIT x "

export default function YcStartup() {
  useEffect(() => {
    void trackEvent("partner_page_view", { source: SOURCE, path: "/yc-startup" })
  }, [])
    ...
    <div className="ycs">
      ...
      <header className="ycs-nav">
        ...
        <div className="ycs-event-chip" aria-label="For attendees of YC Startup School 2026">
          ...
          <small>MATCHING: JULY 25TH, 7PM</small>
        </div>
...
.ycs-sun::before {
  ...
  background:
    radial-gradient(ellipse at 50% 88%,
      rgba(247, 83, 28, 0.30) 0%,
      rgba(247, 83, 28, 0.17) 28%,
      rgba(247, 83, 28, 0.07) 48%,
      rgba(247, 83, 28, 0.02) 62%,
      transparent 76%);
  -webkit-mask-image: radial-gradient(ellipse at 50% 88%,
    #000 0%,
    rgba(0, 0, 0, 0.72) 38%,
    rgba(0, 0, 0, 0.20) 58%,
    transparent 74%);
  mask-image: radial-gradient(ellipse at 50% 88%,
    #000 0%,
    rgba(0, 0, 0, 0.72) 38%,
    rgba(0, 0, 0, 0.20) 58%,
    transparent 74%);
...
.ycs-sun-core {
  ...
  inset: -20% -34% -4px;
  background:
    repeating-linear-gradient(90deg, rgba(255, 246, 232, 0.11) 0 1px, transparent 1px 23px),
    radial-gradient(ellipse at 50% 100%,
      #F74312 0%,
      #FF4C18 25%,
      #FF6730 43%,
      rgba(255, 125, 78, 0.48) 56%,
      rgba(255, 154, 112, 0.14) 68%,
      transparent 80%);
  ...
  filter: blur(2.8px);
  box-shadow:
    0 -44px 118px rgba(247, 83, 28, 0.24),
    0 22px 88px rgba(247, 83, 28, 0.24);
  z-index: 1;
}
.ycs-sun-copy {
  position: relative;
...
.ycs-sun-tick { font-size: 12px; letter-spacing: 0.18em; opacity: 0.92; }
