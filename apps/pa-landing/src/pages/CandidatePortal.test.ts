...
test("CandidatePortal /me lets signed-in users connect an existing Claire phone thread", () => {
  ...
  assert.match(source, /<MePhoneThreadLinkCard phoneVerified=\{profileHasVerifiedPhone\(profile\)\} \/>/)
  assert.doesNotMatch(source, /!\s*profileHasVerifiedPhone\(profile\) \? <MePhoneThreadLinkCard/)
  assert.match(source, /function MePhoneThreadLinkCard\(\{ phoneVerified \}: \{ phoneVerified: boolean \}\)/)
  assert.match(source, /I've texted Claire/)
  assert.match(source, /Switch to that phone thread\./)
