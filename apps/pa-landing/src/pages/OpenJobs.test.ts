...
test("OpenJobs mobile chrome uses responsive wrappers instead of forcing page-wide overflow", () => {
  ...
  assert.match(source, /showLayoutSwitcher=\{!isNarrowViewport\}/)
  assert.match(source, /\{showLayoutSwitcher \? <LayoutSwitcher value=\{layout\} onChange=\{setLayout\} \/> : null\}/)
  assert.match(source, /className="open-layout-switcher" role="group" aria-label="Role list layout"/)
  ...
  assert.doesNotMatch(source, /@media \(max-width: 720px\)[\s\S]*\.open-layout-switcher[\s\S]*width: 100%/)
