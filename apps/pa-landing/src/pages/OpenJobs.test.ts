...
test("OpenJobs mobile layout gets candidates to real role cards before filter chrome dominates", () => {
  assert.match(source, /@media \(max-width: 720px\)[\s\S]*\.open-main-section[\s\S]*padding-top: 18px !important/)
  ...
  assert.match(source, /@media \(max-width: 720px\)[\s\S]*\.open-tab[\s\S]*padding: 11px 12px 12px !important/)
