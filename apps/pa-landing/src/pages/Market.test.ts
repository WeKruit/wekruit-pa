...
test("Market role briefs can be searched before candidates scan the full inventory", () => {
  ...
  assert.match(source, /aria-label="Search role briefs"/)
...
test("Market default role-brief tab shows the Claire operating lane before role inventory", () => {
  assert.match(source, /function MarketRoleBriefContract\(\)/)
  assert.match(source, /function MarketOperatingLane\(\{ kind, count \}: \{ kind: "direct" \| "hunting"; count\?: number \}\)/)
  assert.match(source, /aria-label="Claire marketplace operating lane"/)
  assert.match(source, /Roles become Claire screens, not another application queue\./)
  assert.match(source, /\.wk-shell \.wk-market-lane__intro h2 \{[\s\S]*line-height: 1\.22;[\s\S]*letter-spacing: 0;/)
  assert.doesNotMatch(source, /\.wk-shell \.wk-market-lane__intro h2 \{[\s\S]*line-height: 1\.08;/)
  assert.match(source, /<strong>Profile<\/strong>/)
  assert.match(source, /Role brief/)
  assert.match(source, /Role signal/)
  assert.match(source, /Employer must-haves set Claire's first screen\./)
  assert.match(source, /External postings help tune what Claire should watch\./)
  ...
  assert.match(source, /<MarketOperatingLane kind="direct" count=\{direct\.isSuccess \? directJobs\.length : undefined\} \/>[\s\S]*\{direct\.isPending \? \([\s\S]*<div className="wk-direct-toolbar" aria-label="Role brief search">[\s\S]*<div className="wk-direct-cards">[\s\S]*<DirectCard key=\{r\.id\} r=\{r\} onTalk=\{\(\) => onTalkToClaire\(r\)\} \/>[\s\S]*\)\}[\s\S]*<MarketRoleBriefContract \/>/)
  assert.match(source, /<MarketOperatingLane kind="hunting" count=\{hunting\.isSuccess \? huntingTotal : undefined\} \/>[\s\S]*\{trackedRolesEmpty \? \(/)
  assert.match(source, /href="\/me\/profile#profile-corrections"[\s\S]*Update profile signals/)
  assert.doesNotMatch(source, /MarketRoleBriefWorkflow/)
...
test("Market keeps mobile role cards primary while preserving the compact support contract", () => {
  ...
  assert.match(source, /<MarketOperatingLane kind="direct" count=\{direct\.isSuccess \? directJobs\.length : undefined\} \/>[\s\S]*<div className="wk-direct-toolbar" aria-label="Role brief search">[\s\S]*<div className="wk-direct-cards">[\s\S]*<DirectCard key=\{r\.id\} r=\{r\} onTalk=\{\(\) => onTalkToClaire\(r\)\} \/>[\s\S]*<MarketRoleBriefContract \/>/)
  ...
  assert.match(source, /@media \(max-width: 720px\) \{[\s\S]*\.wk-shell \.wk-market-lane \{[\s\S]*padding: 13px 0 12px;[\s\S]*display: block;/)
  assert.match(source, /@media \(max-width: 720px\) \{[\s\S]*\.wk-shell \.wk-market-lane__steps \{[\s\S]*display: grid;[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\);/)
  assert.match(source, /\.wk-shell \.wk-market-lane__steps em \{[\s\S]*display: none;/)
})
...
test("Market mobile headlines leave enough line height for two-line Newsreader italics", () => {
  ...
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market__h1 \{[\s\S]*font-size: 34px;[\s\S]*line-height: 1\.16;[\s\S]*margin: 10px 0 12px;[\s\S]*\.wk-shell \.wk-market__h1--direct \{[\s\S]*gap: 4px;/)
...
test("Market tab chrome separates labels, counts, and subtitles on narrow screens", () => {
  assert.match(source, /const accessibleLabel = `\$\{label\}\$\{count !== undefined \? `, \$\{count\}` : ""\}\. \$\{sub\}\.`/)
  assert.match(source, /<button[\s\S]*id=\{id\}[\s\S]*type="button"[\s\S]*className=\{`wk-mtab\$\{active \? " is-active" : ""\}`\}/)
  assert.match(source, /aria-controls=\{controls\}/)
  assert.match(source, /aria-label=\{accessibleLabel\}/)
  assert.match(source, /<div className="wk-mtabs" role="tablist" aria-label="Market role source">/)
  assert.match(source, /id="market-tab-direct"[\s\S]*controls="market-panel-direct"/)
  assert.match(source, /id="market-tab-hunting"[\s\S]*controls="market-panel-hunting"/)
  assert.match(source, /id="market-panel-direct"[\s\S]*role="tabpanel"[\s\S]*aria-labelledby="market-tab-direct"/)
  assert.match(source, /id="market-panel-hunting"[\s\S]*role="tabpanel"[\s\S]*aria-labelledby="market-tab-hunting"/)
  assert.match(source, /aria-label="Search tracked roles"/)
...
test("Market narrow phone breakpoint keeps compact tabs and readable operating-lane rows", () => {
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-mtabs \{[\s\S]*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\);[\s\S]*gap: 8px;[\s\S]*padding-top: 10px;[\s\S]*margin-bottom: 16px;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-mtab \{[\s\S]*min-height: 74px;[\s\S]*border: 1px solid var\(--wk-border\);[\s\S]*border-radius: 10px;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-mtab__top \{[\s\S]*justify-content: space-between;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-mtab__label \{[\s\S]*font-size: 18\.5px;[\s\S]*white-space: normal;/)
  assert.match(source, /@media \(max-width: 720px\) \{[\s\S]*\.wk-shell \.wk-market-lane__intro h2 \{[\s\S]*font-size: 21px;[\s\S]*line-height: 1\.28;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market-lane__intro h2 \{[\s\S]*font-size: 20px;[\s\S]*line-height: 1\.26;[\s\S]*max-width: 14\.5em;/)
  assert.doesNotMatch(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market-lane__intro h2 \{[\s\S]*line-height: 1\.18;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market-lane__steps \{[\s\S]*display: grid;[\s\S]*grid-template-columns: repeat\(4, minmax\(0, 1fr\)\);/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market-lane__steps em \{[\s\S]*display: none;/)
...
test("Market narrow phone role-brief inventory keeps the first role actionable in the first viewport", () => {
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market__panel--direct \.wk-market__head \{[\s\S]*margin-bottom: 12px;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market__panel--direct \.wk-eyebrow \{[\s\S]*font-size: 10\.5px;[\s\S]*line-height: 1\.2;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-market__lede \{[\s\S]*font-size: 14\.5px;[\s\S]*line-height: 1\.34;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-direct-cards \{[\s\S]*gap: 8px;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-direct-card \{[\s\S]*padding: 13px;[\s\S]*gap: 8px;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-direct-card__owner \{[\s\S]*display: none;/)
})
