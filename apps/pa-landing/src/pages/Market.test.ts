...
test("Market default role-brief tab shows role inventory before the supporting Claire contract", () => {
  ...
  assert.match(source, /\.wk-shell \.wk-roleflow__intro h2 \{[\s\S]*line-height: 1\.22; letter-spacing: 0;/)
  assert.doesNotMatch(source, /\.wk-shell \.wk-roleflow__intro h2 \{[\s\S]*line-height: 1\.08;/)
...
test("Market narrow phone breakpoint keeps compact tabs and restores readable workflow rows", () => {
  ...
  assert.match(source, /@media \(max-width: 720px\) \{[\s\S]*\.wk-shell \.wk-roleflow__intro h2 \{[\s\S]*font-size: 21px;[\s\S]*line-height: 1\.32;/)
  assert.match(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-roleflow__intro h2 \{[\s\S]*font-size: 23px;[\s\S]*line-height: 1\.34;[\s\S]*max-width: 13em;/)
  assert.doesNotMatch(source, /@media \(max-width: 420px\) \{[\s\S]*\.wk-shell \.wk-roleflow__intro h2 \{[\s\S]*line-height: 1\.18;/)
