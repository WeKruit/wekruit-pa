...
test("ConnectLinkedin missing-token state does not render a dead URL form", () => {
  ...
  assert.match(source, /CLAIRE_PHONE_LINK_LOGIN\s*=\s*"\/login\?next=%2Fme&phoneLink=1"/)
  assert.match(source, /href=\{CLAIRE_PHONE_LINK_LOGIN\}[\s\S]*I&apos;ve texted Claire/)
  assert.match(source, /href="\/me"[\s\S]*Go to My WeKruit/)
  assert.match(source, /href="\/onboarding"[\s\S]*Start onboarding instead/)
  assert.match(source, /\.wk-li-connect__actions \.wk-li-connect__link \{[\s\S]*margin-top: 0;/)
