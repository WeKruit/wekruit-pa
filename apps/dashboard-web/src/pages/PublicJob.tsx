...
const SENDBLUE_NUMBER = "+18667700087" // production Sendblue from-number
...
export default function PublicJob() {
  ...
  const smsHref = `sms:${SENDBLUE_NUMBER}?body=${encodeURIComponent(smsBody)}`
  const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(smsHref)}`
  ...
  return (
    <Page>
      ...
      <p style={{ fontSize: "0.75em", color: "#a59781", marginTop: "2rem" }}>
        ...
        <a href="/legal">privacy &amp; terms</a>. WeKruit will text you from {SENDBLUE_NUMBER}.
      </p>
    </Page>
  )
}
