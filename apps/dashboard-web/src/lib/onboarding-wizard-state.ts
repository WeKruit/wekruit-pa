// Onboarding Wizard — client-side state (localStorage) for the FIRST slice of
// the enterprise-onboarding UX (Module A). A server-state callable
// (`paOnboardingState`) is a later slice; for now the wizard is fully
// resumable from the browser. This file is the single source of truth for the
// milestone list, the persisted shape, and the load/save helpers so both the
// wizard shell and the checklist home read identical data.

export const WIZARD_STORAGE_KEY = "wekruit-onboarding-wizard-v1"
...
export function resolvedCount(state: WizardState): number {
  // done OR skipped — used for the checklist completion ring.
