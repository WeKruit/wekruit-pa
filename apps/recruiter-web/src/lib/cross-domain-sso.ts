 ...
 * user who signed in on a WeKruit-owned subdomain stays signed in when they
 * navigate to another WeKruit-owned subdomain without having to re-authenticate.
 *
 * Three pieces:
...
function shouldUseCrossDomainSso(): boolean {
  ...
  return window.location.hostname === "wekruit.com" || window.location.hostname.endsWith(".wekruit.com")
}
