 ...
 * the thin agent runs the 5 canonical SHARED onboarding questions IN ORDER, and the AGENT calls the
...
const EXPECT_SLOT = [null, "main_goal", "culture_stage", "industry_interest", "location_relocation", "special_context"]
...
async function main() {
  ...
  for (let i = 1; i <= 5; i++) ck(`turn${i + 1}: answers slot '${EXPECT_SLOT[i]}'`, turns[i].slot === EXPECT_SLOT[i], `got ${turns[i].slot}`)
  // RELIABILITY: every answer turn must actually advance durable state = the agent called the tool.
  const answerTurns = turns.slice(1, 6)
  const recordedCount = answerTurns.filter((t) => t.recorded).length
  ck("reliability: agent called record_onboarding_answer on ALL 5 answer turns (durable advanced)", recordedCount === 5, `${recordedCount}/5 advanced`)
  ...
  ck("final: all 5 slot answers recorded (write-through accumulation)", ["main_goal", "culture_stage", "industry_interest", "location_relocation", "special_context"].every((s) => answers[s]), `[${Object.keys(answers).join(",")}]`)
  ...
  const allReplies = turns.slice(0, 6).map((t) => t.reply.toLowerCase()).join("\n")
  ck("voice: no markdown in onboarding questions", !/[*_`]|^\s*[-•]/m.test(allReplies), "markdown found")
  ck("coverage: Claire asked a question on turns 1-5", turns.slice(0, 5).every((t) => t.reply.length > 0))
