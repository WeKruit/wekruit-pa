...
/** 1:1 iMessage session key. Email/iCloud handles must remain handles; phone handles normalize to E.164. */
export function getImessageSessionExternalId(participant: string, chatId: string): string {
  if (process.env.PA_IMESSAGE_SESSION_KEY === "chatid") {
    return chatId
  }
  return normalizeImessageParticipant(participant)
}
