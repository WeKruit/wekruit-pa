...
**Estimate:** ~10–14 dev-days across **11 phases (52–62)** — **shipped 2026-05-06** (single-day execution wave).

**Shipped:** 2026-05-06. Roster + verification: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md). Per-phase context: `.planning/phases/*/*-CONTEXT.md`.

**Phases:**

- [x] **Phase 52: Canonical Tag Vocab Foundation** — `packages/shared-tags` extends 10 axes, all spelled-out, zod write-time validation. Foundation for all downstream phases. (`5d1c603`)
- [x] **Phase 53: pa-resume-parser v2 wire + relevantTags extract** — cv-ingest wires v2 `parseResumeText`, schema gains `relevantIndustry` / `relevantSpecialization` / `proposedTags`, idempotency + Sonnet-4-6 fallback in chain. (`3209bc5`)
- [x] **Phase 54: Unified pa-users.tags writer** — single source-of-truth user tag store, `mergeUserTags()` only writer, migration script for 100+ existing users. (`d693f81`)
- [x] **Phase 55: matching-jobs schema migration + roleFunction backfill** — Firestore schema gains `roleFunction[]`, deterministic 116K-job migration from `industry` → 17 jobright role functions. (`5e74248`)
- [x] **Phase 56: queryMatchingJobs read pa-users.tags + filter + score** — generateJobRecs reads exclusively from pa-users.tags, hard-filter chain + soft score weights + per-job reasoning with top-2 weighted skills. (`6adb9b8`)
- [x] **Phase 57: Liveness/404 sweep + atsApplyUrl backfill** — daily 404 sweep CF + 30K active in <60min, dead>30d hard-delete, paBackfillMatchingJobsAtsUrl wired into daily. (`57c182b`)
- [x] **Phase 58: Nightly LLM rerank batch + per-skill JD-rel weight** — Cloud Scheduler 03:00 UTC Qwen-7B JSON-mode batch, `pa-user-rerank-cache` + `pa-user-skill-jdrel-cache`, fire-and-forget reuse. (`463bcdb`)
- [x] **Phase 59: Dashboards (canonical-tags + qa-evaluator + onboarding-questions extension)** — `/admin/canonical-tags` with sandbox-promotion + counts; `/admin/qa-evaluator` weekly results; `/admin/onboarding-questions` per-user pa-users.tags view. (`661a039`)
- [x] **Phase 60: Dev triggers + scenarios + fixtures** — `__PA_FIND_MATCH__` mirrors `__PA_RESET__`, scenario runner `--user-id` flag, dump tail rerank-cache flag, 5-persona fixture set. (`7499a1b`)
- [x] **Phase 61: QA evaluator thread weekly run** — Cloud Scheduler `paQaEvaluatorWeekly` Mon 09:00 UTC, 100 user×match pair sample, hard-filter pass + top-3 acceptable, failure-loop until ≥90%/70% pass. (`12a5934` — infra shipped; pass-rate signal data-calibrated per audit)
- [x] **Phase 62: Documentation (CLAUDE.md / MILESTONE-v1.6.md / cross-repo handoff)** — milestone doc, CLAUDE.md design lock, packages/shared-tags README, wekruit-scraping `WEKRUIT_PA_TAG_HANDOFF.md`. (`eab4e63`)

**Phase summary table:**

| # | Phase | Goal | Requirements | Quantitative Gate | Status |
|---|-------|------|--------------|-------------------|--------|
| 52 | Canonical Tag Vocab Foundation | Extend `packages/shared-tags` with 10 axes (roleFunction 17 / industrySector 42 / major 45 / visa 4 / jobType 10 / careerStage 13 / location 130+ / relevantTags / skills / sandbox-promote), all spelled-out, zod write-time validation, Firestore overlay readable. | TAG-01..12 | Zod write-time rejects abbreviations + non-canonical tokens; all 10 axes exported with type tests | Shipped `5d1c603` |
| 53 | pa-resume-parser v2 wire + relevantTags extract | cv-ingest uses `pa-resume-parser` v2 `parseResumeText`; schema extended with `relevantIndustry`, `relevantSpecialization`, `proposedTags` (max 12); LLM chain gpt-5.4-nano → claude-sonnet-4-6 → gpt-4.1-mini; post-parse Claire dialogue confirm; idempotent on sha256. | PARSE-01..09 | Adam re-parse round-trip yields `industryTags = ["artificial_intelligence_and_machine_learning", "technology_general"]` (not `["other"]`); idempotent on sha256 | Shipped `3209bc5` |
| 54 | Unified pa-users.tags writer | Single source-of-truth `pa-users/{userId}.tags`. cv-ingest + chat-answer hooks both go through `mergeUserTags()` (iter34 H.1). Migration script for 100+ users. Inconsistency surfaced to admin. | USER-TAG-01..05 | All 100+ existing users have populated `pa-users.tags`; only `mergeUserTags()` is the writer (grep shows zero direct writes) | Shipped `d693f81` |
| 55 | matching-jobs schema migration + roleFunction backfill | Add `roleFunction: string[]` to `matching-jobs`, retain `industrySector`. Deterministic mapper backfills 116K+ jobs from current `industry` → 17 jobright role functions. | MATCH-02 | 116K+ `matching-jobs` rows have `roleFunction[]`; migration is idempotent + dry-run audit shows ≤1% unmapped | Shipped `5e74248` |
| 56 | queryMatchingJobs read pa-users.tags + filter + score | generateJobRecs reads exclusively from `pa-users.tags` (no legacy fallback). Firestore `array-contains-any roleFunction`, limit 500. Hard post-filter chain (visa/loc/stage/jobType/firstSeenAt<20d/atsApplyUrl/dead). Soft score weights (llm 0.40 / skill 0.20 / relevant 0.15 / industry 0.10 / emb 0.10 / salary 0.05). Per-skill base × JD-relative weight. Per-job reasoning shows top-2 weighted skills. | MATCH-01, MATCH-03..08 | SWE Adam scenario yields 0 BDR/sales/cashier recs; per-job reason surfaces top-2 weighted JD-aligned skills; `lastSeenAt` not read (replaced by 20d firstSeenAt window) | Shipped `6adb9b8` |
| 57 | Liveness/404 sweep + atsApplyUrl backfill | Daily Cloud Scheduler HEAD-checks `matching-jobs.atsApplyUrl`, marks dead, hard-deletes >30d. Reuses `paBackfillMatchingJobsAtsUrl` (iter34 G.3). | LIVE-01..04 | jobright.ai-leaked recommendation rate: 50%+ → 0%; sweep clears 30K active in <60min | Shipped `57c182b` |
| 58 | Nightly LLM rerank batch + per-skill JD-rel weight | Cloud Scheduler 03:00 UTC Qwen-7B JSON-mode batch. `pa-user-rerank-cache/{userId}` (top-50/active user). Per-skill JD-rel cache `pa-user-skill-jdrel-cache/{userId}/{jobId}`. Reuses fire-and-forget llmRerank (iter34 H.2). | RERANK-01..04 | Nightly batch completes in <24h for all active users; cache stale-by >36h triggers fallback | Shipped `463bcdb` |
...
All items **passed at ship** 2026-05-06 (evidence + caveats in [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md)).

- [x] Phase 52 vocab foundation locked + zod schema rejects abbreviations
- [x] Phase 53 pa-resume-parser v2 wired + Adam re-parse yields correct industry tags
- [x] Phase 54 unified `pa-users.tags` migration complete (100+ users, 0 direct-writers)
- [x] Phase 55 `matching-jobs` schema migration complete (116K+ rows backfilled)
- [x] Phase 56 generateJobRecs read-side cutover; SWE Adam scenario yields 0 BDR/sales recs
- [x] Phase 57 daily 404 sweep stable; jobright.ai leak rate 0%
- [x] Phase 58 nightly Qwen-7B rerank batch operational <24h
- [x] Phase 59 admin dashboards live; sandbox→canonical promotion verified
- [x] Phase 60 `__PA_FIND_MATCH__` + 5-persona fixtures committed
- [x] Phase 61 weekly QA evaluator: infra live; threshold pass rates data-calibrated (first runs sample-thin — see audit)
- [x] Phase 62 docs landed (CLAUDE.md / MILESTONE-v1.6.md / shared-tags README / cross-repo handoff)
...
**Status:** Shipped 2026-05-06 (`5d1c603`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`3209bc5`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`d693f81`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`5e74248`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`6adb9b8`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`57c182b`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`463bcdb`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`661a039`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`7499a1b`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`12a5934`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Status:** Shipped 2026-05-06 (`eab4e63`). Verified: [v1.6-MILESTONE-AUDIT.md](v1.6-MILESTONE-AUDIT.md).
**Success Criteria**:
...
**Phase numbering:** continues from v1.6 last phase 62. Baseline roadmap: phases **63–72** (10 phases). **Phase 73** (career-ops direct API harvest on macmini) landed same sprint — [.planning/STATE.md](STATE.md).

**Shipped:** 2026-05-06 (63–72) + Phase 73. Roster + verification: [.planning/v1.7-MILESTONE-AUDIT.md](v1.7-MILESTONE-AUDIT.md). Phase 73 operational notes: [.planning/STATE.md](STATE.md).

| # | Phase | Reqs | Status |
|---|-------|------|--------|
| 63 | LinkedIn / Wellfound senior-job scraper | SENIOR-01..05 (5) | Shipped (`7c83f62` + macmini `60359a4`) |
| 64 | Sponsorship LLM inference + allowlist | SPONSOR-01..05 (5) | Shipped (combined) |
| 65 | Serper backfill batch parallelism + retry | ATSURL-01..04 (4) | Shipped (`a0b6029`) |
| 66 | Macmini Stage 2.5 permanent fix | MACMINI-01..03 (3) | Shipped (`6caee56` + macmini `b81ecaf`) |
| 67 | Launchd reliability + health-check | LAUNCHD-01..03 (3) | Shipped (`7bb9cb0`) |
| 68 | Vocab hygiene closure | HYGIENE-01..04 (4) | Shipped (`a7bf6c5`) |
| 69 | Secrets + Slack alert provisioning | SECRETS-01..03 (3) | Shipped (`d26b3fa`) |
| 70 | Match-debug admin UI | MATCHDEBUG-01..04 (4) | Shipped (`87bb878`) |
| 71 | QA data ramp (auto-derive tags) | QADATA-01..04 (4) | Shipped (`b0a9c39`) |
| 72 | Documentation v1.7 | DOC-V17-01..02 (2) | Shipped (`c3f1120`, `d9722f6`) |
| 73 | Career-ops port — GH/Lever/Ashby direct APIs | SENIOR-V2-01..06 (6) | Shipped (see STATE: macmini `2bbaa9a`; wekruit-pa `c3e786e`) |
...
**Status:** Shipped 2026-05-06 (combined). Verified: [.planning/v1.7-MILESTONE-AUDIT.md](v1.7-MILESTONE-AUDIT.md).
**Success Criteria:**
