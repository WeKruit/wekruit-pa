...
Phase: Stream A v1.3 carryover ✅ COMPLETE (29 + 30 + 31 + 32). Stream C v1.4 in progress — Phases 33 + 34 ✅ COMPLETE; Phase 35 ✅ PARTIAL (detectors built, wire-in Adam-owed); Phase 36 ✅ PARTIAL (injector + A/B harness built, wire-in Adam-owed); Phase 37 ✅ PARTIAL (FSM built, wire-in Adam-owed); Phase 38 ✅ PARTIAL (Memory Policy built, wire-in Adam-owed); Phase 39 ✅ PARTIAL (External Benchmarks BUILD complete, RUNS Adam-owed after $25 approval).
Plan: Stream A done (4/4 phases shipped); Stream C 7/8 (Phases 33 + 34 + 35-partial + 36-partial + 37-partial + 38-partial + 39-partial; 1 remaining: 40 Bible v7.5 + Ship)
Status: Stream A wrapped — operator-grade dashboard + connectors + handbook live; Stream C — F1/F2/F3/F4 detector module + ImperfectionInjector + 3-arm A/B harness + FSM (5 UX × ESConv 8) module + Memory Policy (advice tracker + contradiction detector + extractor pinning) + External Benchmarks harness (5 runners + 3 adapters + cost ledger $25 cap + run-all orchestrator) shipped behind feature flags `PA_DETECTORS_ENABLED`, `PA_IMPERFECTION_INJECTOR_ENABLED`, `PA_FSM_ENABLED`, and `PA_MEMORY_POLICY_ENABLED` (default off); Adam-owed P0 tasks: apply 4 WIRE-IN-PATCH.md files in order (35 → 36 → 37 → 38) after committing pending uncommitted work + approving $0.50-$2 LLM budget for live A/B run + verify Firestore composite index for `pa-advice-tracker` + approve $25 budget for Phase 39 external benchmark runs + clone 5 benchmark repos per Phase 39 SETUP.md + run `node apps/eval/external-benchmarks/run-all.mjs --live --arm=...` (~3hr per arm × 3 arms)
Last activity: 2026-04-30 — Phase 39 T1-T5 complete (P9-C); 88/88 external-benchmarks tests pass; typecheck clean; `node run-all.mjs --dry-run` exit 0 with 5 runner plans + total_projected_cost_usd $0.232 (claire-stack) + budget_headroom $24.77; cost projection $2.41 grand total (qwen-7b $0.21 + qwen-72b $1.78 + claire-stack $0.23 + optional judge $0.18) vs $25 cap; cost ledger BudgetExceededError abort path verified; SETUP.md + COST-PROJECTION.md written for Adam handoff
