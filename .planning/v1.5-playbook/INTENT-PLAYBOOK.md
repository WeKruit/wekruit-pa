...
### Gap 3.12 — `typing_indicator` was audit-only; coalesce was time-driven, not event-driven ⚠ — **FIXED 2026-05-03 (Adam 顶层设计)**

**Original evidence (preserved for historical context):** `apps/functions/src/sendblue/webhook.ts:331-350` (pre-fix) routed every `typing_indicator` Sendblue event to `safeAudit({type:"typing_indicator_received"}) → reply 200 ignored`. The audit log existed; the signal it carried did not flow into the coalesce buffer. Adam实测 4 zh msgs → 6 reply bubbles: natural typing gaps > `DEFAULT_DELAY_MS=4_000` (pre-Bug-4) wave-split a single thought into 3 turns, each turn produced a partial reply. Bug 4 (commit 1145f13) bumped the window 4s→8s as a fallback baseline; this gap closes the loop with a primary event-driven path.

**Resolution (2026-05-03):**
- **Schema verified.** Sendblue typing webhook payload (per `docs.sendblue.com/api-v2/typing-indicators`, queried 2026-05-03): `{ number, is_typing, from_number, timestamp }`. CRITICAL: `number` = the user typing (E.164), `from_number` = the Sendblue line. Direction is REVERSED relative to inbound `receive` events. Older deliveries may omit `is_typing`; treat missing as `true` (conservative — extend window).
- **Detection:** `apps/functions/src/sendblue/types.ts` extends `SendblueTypingIndicatorPayload` with `is_typing` / `from_number` / `timestamp` fields (still permissive — `[key: string]: unknown` preserved for forward compat).
- **Bump path (pure transaction):** `apps/functions/src/coalesce/buffer.ts:bumpCoalesceBufferTransaction` reads the user's current turnSeq, finds the active pending buffer, atomically patches `lastReceivedAt` + a fresh `pendingTaskName` (suffix uses `messageCount * 100 + lastTypingTickSeq` to avoid Cloud Tasks ALREADY_EXISTS within the ~1h tombstone window). Returns one of `{ bumped, no-buffer, hard-capped, fired }`. **`firstReceivedAt` is NEVER mutated** — `HARD_CAP_MS=12_000` remains anchored to the moment the FIRST inbound message landed. This is the anti-troll invariant: continuous typing CANNOT keep a turn pending past 12s.
- **Bump path (Cloud Tasks orchestration):** `apps/functions/src/coalesce/paMessageCoalescer.ts:bumpCoalesceBuffer` wraps the transaction with cancel-prior-task + enqueue-new-task. Two delays:
  - `is_typing=true` → `TYPING_BUMP_DELAY_MS=8_000` (= `DEFAULT_DELAY_MS`, intuitive: "keep waiting, user is still typing")
  - `is_typing=false` → `TYPING_STOPPED_TAIL_MS=2_000` (short tail tolerates the lag between "fingers off keys" and "tap send"; never zero, because the user typically taps send a moment after stopping)
  - Both clamped to `min(desired, hardCap - elapsed)`.
- **Webhook wire-in:** `apps/functions/src/sendblue/webhook.ts` typing branch resolves `payload.number` → `userId` via `defaultLookupUserByPhone`, gates on `paMessageCoalesceEnabled` (perUser scope, same flag as the inbound coalesce path), and calls `bumpCoalesceBuffer` with `{ userId, isTyping }`. Fail-open in every layer: phone-lookup miss → log + audit only; flag off → audit only; bump throws → log + audit only. Audit row `type=typing_indicator_received` now correctly populates `fromNumber` from `payload.number` (the user) instead of `payload.from_number` (Claire) — pre-fix the audit logged the wrong direction.
- **Fail-open posture:** typing is a HINT, never a SOURCE OF TRUTH. The audit + 200 OK path always runs; if Cloud Tasks rejects the cancel/enqueue, the prior task fires on its original schedule and the buffer-sweep (`buffer-sweep.ts`, R1 mitigation) catches stuck buffers within `staleAfterMs=30_000`. Worst case: turn fires via sweep ~30s instead of the bumped 8s — degrades to time-driven coalesce, no user-visible loss.
- **Tests:** 6 unit + integration in `apps/functions/src/coalesce/__tests__/typing-event.test.ts` covering: bump-during-open-buffer extends delay, no-buffer is no-op, typing_stopped fires at TYPING_STOPPED_TAIL_MS, hard-cap respected even with continuous typing, fired-buffer ignored, webhook integration calls bumpCoalesceBuffer + audits with the correct phone direction. All 376 functions tests still pass (no regression).

**Subscription dependency (operator action, NOT code):** Sendblue dashboard MUST have `typing_indicator` event subscribed for the receive webhook URL. Recent production logs (`gcloud functions logs read paSendblueWebhook --limit=500`) show ZERO typing-indicator deliveries — likely the dashboard subscription is missing or disabled. The code is now ready; Adam needs to verify the subscription is on. Until then, the `DEFAULT_DELAY_MS=8_000` baseline (Bug 4) remains the only coalesce signal.

**Telemetry:** every bump produces a structured `[coalesce] bump` log with `{userId, isTyping, action, taskName, delayMs, turnSeq}`. Dashboards can compute hit rate as `count(bumped) / count(typing_indicator_received)`.
